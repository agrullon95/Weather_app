$(document).ready(function() {
  if (navigator.geolocation) {
    ...
    navigator.geolocation.getCurrentPosition(function(position) {
        ...
        $.getJSON("http://api.openweathermap.org/data/2.5/weather?lat=" + lat + "&lon=" + long + "&APPID=9d11827e45e418fb50d4d97502e40560&units=imperial", function(result) {
          $('.location').html(result.name); //location
          $('.humidity').append(result.main.humidity + "%"); // humidity
          $('.degree').attr('data-value', Math.round(result.main.temp)); // temperature
          $('.degree').text(Math.round(result.main.temp) + String.fromCharCode(176) + "F"); //temperature fahrenheit
          $('.main').html(result.weather[0].main); // condition
          $('.windy').attr('data-value', result.wind.speed);
          console.log($('.windy').attr('data-value'));
          $('.windy').append(result.wind.speed + " mph"); // wind
          $('.icon').attr('src', 'http://openweathermap.org/img/w/' + result.weather[0].icon + '.png'); //weather icon
  ...
  $('.amPm').text(moment().format('a'));

  // updates time
  var myTime = setInterval(myTimer, 500);
  ...
  // Date
  $('.month').text(moment().format('MMMM Do'))
...
$('.unit').on('click', function() {
  var fahren = $('.degree').attr('data-value');
  var celcius = Math.round((fahren - 32) / 1.8) + String.fromCharCode(176) + "C";
  var mph = $('.windy').attr('data-value');
  var kph = (mph * 1.60934).toFixed(2);
  if (units === "f") {
    units = "c";
    $('.degree').text(celcius);
    $('.unit').text("To Fahrenheit");
    $('.windy').text("Wind: "+ kph + " KPH");
  } else {
    units = "f";
    $('.degree').text(fahren + String.fromCharCode(176) + "F");
    $('.unit').text("To Celcius");
    $('.windy').text("Wind: "+ mph + " MPH");
  }
});
